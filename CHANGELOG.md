...
- One-time, resumable, copy-not-move migration on first activation with server storage present: existing `localStorage` values are copied into `marinara.storage` per key (never overwriting an existing server entry, where a stored `null` counts as present — i.e. a tombstone is never resurrected from `localStorage`), gated by a per-device `mrr-migrated-v1` flag.
- `marinara.storage.delete()` (which removes the *entire* extension record, not a single key) is never called; per-key deletion uses a `patch({key: null})` tombstone, treated as present-but-absent on every read and by migration.
...
- `COUPLINGS.md` row 1 updated with the verified `get`/`patch`/`delete` contract (shallow-merge patch, null-stores-as-tombstone, 1,000,000-byte cap on the merged record, whole-record-nuke `delete`).
