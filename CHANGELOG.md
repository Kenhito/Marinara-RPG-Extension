...
- **⚠️ IMPORTANT LIMIT — READ BEFORE RELYING ON THIS FOR EXALTED-CLASS SYSTEMS:** swipe-revert covers numeric fields (attributes/skills/derived stats via the generic numeric path) and ruleset-declared states ONLY. It does **NOT** undo typed-damage-TRACK hits (Exalted's bashing/lethal/aggravated health boxes and any other `renderAs:"track"` derived stat), nor inventory adds/removes, condition toggles, background edits, intimacies, xp, or attunement/investiture/commitment writes — these are array/object-shaped mutations that are not yet journaled at all, and keep stacking exactly as they did before this feature regardless of swipes. Flagged for extension, not solved this build — queued for a follow-up session, not shipped tonight.
- **Scope boundary (by design, not an oversight):** only the mutation shapes above are journaled/reverted — attributes, skills, derived stats via the generic numeric-field path, and ruleset states. Conditions, inventory, backgrounds, intimacies, xp, attunement/investiture/commitment, and typed-damage-track hits are array/object-shaped mutations that don't reduce to a scalar prev/next pair; they are NOT reverted on a swipe change and keep stacking exactly as they did before this feature (see the B2-R header comment at `extension/RPG-Extension-GM-Mode.js`, in the module region after `saveProcessedMessageIds`, for the full accounting).
