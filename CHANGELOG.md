...
- The narrator path's `applyStateTagsWithDedup` anchor is now the same composite `messageId:swipeIndex` key when the index is known (plain `messageId` otherwise), so identical tag content on two different swipes of the same message both apply while a re-scan of the same swipe still applies once — `applyStateTagsWithDedup` itself is unchanged; the anchor is opaque to it.
- No code path change for the custom-agent runs poller (`pollCustomAgentRuns`) — verified against engine source that a swipe/regeneration always creates a new agent-run row with a fresh id (`runId: newId()` per generation in `generate.routes.ts`), so its existing per-run-id dedup already treats each swipe's run as new work without any keying change.
...
- `marinara.storage.delete()` (which removes the *entire* extension record, not a single key) is never called; per-key deletion uses a `patch({key: null})` tombstone, treated as present-but-absent on every read and by migration/merge.
...
- `COUPLINGS.md` row 1 updated with the verified `get`/`patch`/`delete` contract (shallow-merge patch, null-stores-as-tombstone, 1,000,000-byte cap on the merged record, whole-record-nuke `delete`, and the envelope/newest-wins-merge contract).
