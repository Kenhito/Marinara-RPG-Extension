...
**Step 6b — Add the MRR agent sections to your roleplay preset (engine 2.4.0+, roleplay mode only).** **This step is not optional, and skipping it fails silently.** Since Marinara 2.4.0 a roleplay preset *owns* agent placement: an agent's output is inserted only where a matching **Agent Data** marker section sits in the preset, and with no matching section the engine **discards that agent's output entirely** — no warning, no fallback. The agents still run, still cost tokens, and still show healthy run history, while the narrator never sees a word of it. Easiest fix: **Manage MRR Agents → Add agent sections to active preset** (it names the preset first, skips agents that already have a section, and never writes without your confirmation). By hand: **Preset Editor → Add Section → Agent Sections**. The State Mutator deliberately gets no section — its `[mrr-state: ...]` output is for the extension, which reads it from the run history directly. Game mode and conversation mode don't use preset sections at all. Finally: if Marinara warns that an MRR agent has **no connection configured**, that's a billing/attribution notice, not an error — it is never the cause of missing agent output.
