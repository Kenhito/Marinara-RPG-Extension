...
![Marinara RPG Ruleset dialog. URL field at the top with a raw GitHub URL example, then a multi-line textarea showing a bundle.json paste. Top button row: Manage agents, Import agents (RP-mode-only — see note below). Following button row left-to-right: Fetch URL, Choose file..., Clear, Uninstall server data, Save and reload (highlighted as the primary action). Below that a Library section listing previously installed rulesets with Switch buttons.](docs/screenshots/ruleset-ui.png)

> **Heads-up: the *Manage agents* and *Import agents* buttons at the top of this dialog are for the [Roleplay-Mode sister extension](https://github.com/Kenhito/Marinara-RPG-RP-Mode-Extension) — they do not apply to Game Mode.** The framework's plumbing supports them so the architecture can grow into GM-mode agent imports later, but as of v0.4.0 attempting to import an agents bundle here in GM mode will not register agents you can use in `chatMode: "game"` chats. If you want roleplay-mode agents, install the RP-mode extension. The optional GM-mode sub-agents (described under [Optional sub-agents](#optional-sub-agents-off-by-default) below) install via the ruleset bundle itself, not via these buttons.
