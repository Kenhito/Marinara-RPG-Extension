...
- **Agents bundled and auto-installed.** GM-mode bundles ship the canonical agent pool — Ruleset Helper, `combat-overseer`, `context-fuser`, `state-mutator`, an optional pre-input-transformer, and any per-system parallel-phase overlays (e.g. Exalted's essence-manager, VTM's blood-pool-tracker). All `enabled:true` because GM-mode has no per-agent toggle UI; the bundle IS the install. Idempotent `mrrAgentRole` keying handles re-installs in place.
...
**Step 6b — Add the MRR agent sections to your roleplay preset (engine 2.4.0+, roleplay mode only).** **This step is not optional, and skipping it fails silently.** Since Marinara 2.4.0 a roleplay preset *owns* agent placement: an agent's output is inserted only where a matching **Agent Data** marker section sits in the preset, and with no matching section the engine **discards that agent's output entirely** — no warning, no fallback. The agents still run, still cost tokens, and still show healthy run history, while the narrator never sees a word of it. Easiest fix: **Manage MRR Agents → Add agent sections to active preset** (it names the preset first, skips agents that already have a section, and never writes without your confirmation). By hand: **Preset Editor → Add Section → Agent Sections**. The State Mutator deliberately gets no section — its `[mrr-state: ...]` output is for the extension, which reads it from the run history directly. **You only do this once:** a bundle re-import recreates the agents under new types, which used to orphan every section you'd added, and since round 28 the extension repoints them itself after each import (console: `reconciled N orphaned agent marker(s)`) and re-derives a chat's ruleset stamp if a chat-preset apply wiped it — first-time setup still needs the button, only the re-run is automatic. Game mode and conversation mode skip the preset assembler entirely, so they keep the older depth-0 injection fallback and are unaffected by this step. Finally: if Marinara warns that an MRR agent has **no connection configured**, that's a billing/attribution notice, not an error — it is never the cause of missing agent output. And if a ruleset ships **custom tools**, they only become available when that chat's **enableTools** toggle is on — installing a bundle does not set it for you.
...
- **Percentile under skill** (`d100-percentile`) — Call of Cthulhu (1e-6e), BRP, RuneQuest, Stormbringer, Mythras. 1d100 ≤ skill rating; lower is better. Also covers **roll-high percentile** — Rolemaster / RMSS-family — via `direction: "high"` (roll + bonus vs a target, reported as a total), an informational `bonusFormula`, and an optional `openEnded` block for exploding / imploding open-ended d100 with `unusualFaces` flagging `um=` on the tag. Field-by-field in [`docs/AUTHORING-PHASE-6.md`](docs/AUTHORING-PHASE-6.md) §1.
