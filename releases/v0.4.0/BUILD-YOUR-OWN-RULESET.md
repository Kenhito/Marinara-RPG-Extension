...
```
I want to build a Marinara-RPG-Extension ruleset for <YOUR SYSTEM HERE>.

The framework lives at: https://github.com/Kenhito/Marinara-RPG-Extension

The build documentation is in:
  releases/v0.4.0/docs-for-ai/01-OVERVIEW.md through 07-EXAMPLE-PROMPTS.md
  (read all seven in order)

Two complete worked examples are in:
  releases/v0.4.0/examples/exalted3e/  (complex: typed damage, multi-turn sorcery, three agent overrides)
  releases/v0.4.0/examples/dnd5e/      (simpler: single-roll d20, one agent override)
...
Match the JSON schema in 03-RULESET-SCHEMA.md exactly. Use the system's own vocabulary throughout. Mechanics references only — no verbatim text from published books. GM prompt 2,000-8,000 chars. Lorebook 14-25 entries.
```
...
```
I've uploaded the documentation and two example rulesets for the Marinara-RPG-Extension framework. Read the seven numbered .md files in order. Skim both example folders.
...
Match the schema exactly. Use the system's own vocabulary. Mechanics references only. 2,000-8,000 char GM prompt. 14-25 lorebook entries. Include the "Optional Sub-Agents" lorebook entry verbatim (copy from one of the example bundles).
...
- Open a PR against [Marinara-RPG-Extension](https://github.com/Kenhito/Marinara-RPG-Extension) adding your `rulesets/your-system/` folder.
- Or just publish the `bundle.json` + `agents.json` somewhere (gist, your own repo, Discord) and tell people to grab those two files.

## Done

If your system installs and runs without console errors and the GM agent uses your system's vocabulary correctly, you've shipped a working ruleset. Welcome to system-agnostic Marinara.
