...
`linkedAttribute` is set when the skill always pairs with the same attribute (D&D); omit when the GM picks the attribute per check (Exalted).
...
Min 2 entries. Surfaces in the GM agent prompt as the standard target-number vocabulary.
...
Values' `trigger` text is purely for the GM agent prompt's reference; the extension does not auto-trigger them.
...
The `template` is what the GM agent is told to emit; `example` is a concrete instance. The dice widget produces tags matching this format. Both fields required if `diceTagFormat` is set.
