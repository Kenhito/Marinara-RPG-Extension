...
Beyond this fixed list, Exalted 3e DECLARES its own item fields — `soak_bashing`, `soak_lethal`, `hardness`, `mobility_penalty`, `accuracy`, `defense_bonus`, `overwhelming`, `attunement_cost`, `weapon_tags`, `armor_tags`, `artifact_rating`. Each declared field writes as the snake_case attr of its camelCase storage id; the live list is surfaced every turn in the "Field Reference (extension-managed)" lorebook entry — use the exact attr names that entry gives, never a guessed variant. `mote_commitment` and `mote_pool` ("Personal" or "Peripheral" only) also parse directly on a field="inventory" add tag as a convenience seed when narration establishes an attunement — field="commitment" tags remain the sole enforced path for actually spending and reconciling committed motes. Every declared or convenience attr is type-checked against its declaration: an invalid value (wrong enum option, non-numeric where a number is expected, an unrecognized mote_pool) is SKIPPED, never coerced or guessed into something parseable.
...
4. No cast tag, but the player's message or the narration states the spend in prose ("heightens his senses for 5m personal") — that is a real spend: deduct the stated amount from the pool the text names. If no pool is named, Exalted's default is Personal Motes (subtle; Peripheral is only spent when the text says so or the tag names it).
````
