# Install — Call of Cthulhu 7th Edition ruleset
...
### 2. Install the Call of Cthulhu 7e bundle

Click the **Ruleset** button. The dialog has two ways to load a bundle:

- **Option A — Choose file:** click **Choose file…** and pick
  `rulesets/coc7e/bundle.json` from disk. Click **Save and reload**.
- **Option B — Fetch URL:**
  `https://raw.githubusercontent.com/Kenhito/Marinara-RPG-Extension/main/rulesets/coc7e/bundle.json`

This one import installs the ruleset (sheet + dice widget), the lorebook
("Call of Cthulhu 7e Rules Reference", 68 entries), the main GM agent, four
sub-agents, the `coc7e_reference` custom tool, and two regex scripts that
surface skill-check tags and difficulty mentions in chat. Bundles are data — no
...
2. **Enable the MRR agents for the game** — after it launches, not
   ...
   `MRR: Call of Cthulhu 7th Edition Ruleset Helper` and
   `MRR: Call of Cthulhu 7th Edition — <Role>` (Combat Overseer, Context Fuser,
...
grant to make. Without it the narrating model *invents* every Sanity check,
opposed roll, and pursuit test by picking a plausible number. In a game whose
horror depends on the dice being indifferent, that quietly removes the game.
The same toggle gates this bundle's `coc7e_reference` custom tool.
...
The widget is a roll-under form: **Target**, **Bonus**, and a **Roll 1d100**
button. Enter the skill or characteristic value as Target; success is a roll at
or under it.

- **Difficulty levels are not automatic.** Regular / Hard / Extreme are the full,
  half, and one-fifth skill value. The widget has no difficulty selector — enter
  the reduced number as Target yourself, or leave it at Regular and let the GM
  call the degree from the margin the tag reports.
- **The CRIT SUCCESS label is the Extreme band.** The ruleset sets the critical
  threshold to one-fifth of the target, so a roll that lands there tags as
  `CRIT SUCCESS`. A 1 is not specially marked.
- **Fumbles** are rolls of 96 or higher that also failed.
- **Sanity, Luck, and Magic Points** live on the sheet as tracked resources; the
...
1. Click the dice widget icon. The widget renders the roll-under form: Target,
   Bonus, **Roll 1d100**.
2. Set Target = 65, Bonus = 0. Click **Roll 1d100**.
3. You should see a tag like `[dice: 1d100 vs 65 -> 47 SUCCESS, margin 18]`
   (your dice will vary).
