...
### 2. Install the Trophy Dark bundle

Click the **Ruleset** button. The dialog has two ways to load a bundle:

- **Option A — Choose file:** click **Choose file…** and pick
  `rulesets/trophy-dark/bundle.json` from disk. Click **Save and reload**.
- **Option B — Fetch URL:**
  `https://raw.githubusercontent.com/Kenhito/Marinara-RPG-Extension/main/rulesets/trophy-dark/bundle.json`

This one import installs the ruleset (sheet + dice widget), the lorebook
("Trophy Dark Rules Reference", 19 entries), the main GM agent, three
sub-agents, the `trophy_dark_reference` custom tool, and a regex script that
...
Trophy Dark is a **one-shot** system: the characters are meant to be consumed by
the wood inside a single sitting. Install it into a chat you're willing to end.
...
2. **Enable the MRR agents for the game** — after it launches, not
   ...
   `MRR: Trophy Dark Ruleset Helper` and `MRR: Trophy Dark — <Role>`
   ...
   (the State Mutator is the one agent that writes to the sheet — here, the Ruin
   track). Each enabled agent costs one model call per turn — on a provider that
   allows only one call at a time they run one after another.
...
grant to make. Without it the narrating model *invents* every Risk roll — and a
model asked to guess a number tends to spare the character, which is precisely
the thing Trophy Dark must not do. The doom only lands if the dice are
indifferent. The same toggle gates this bundle's `trophy_dark_reference` custom
tool.
...
The widget is the stance-modal-pool form: a **DARK / LIGHT** segmented control, a
**Pool** field, a **Risk** field, and a **Roll Nxd6** button.

- **Risk is fixed at 3.** The sheet holds it at min = max = 3 because Trophy Dark
  does not vary the threshold per character. Don't edit it.
- **Stance decides the direction.** LIGHT counts each die *over* Risk (4-6) as a
  success — those are your own dice, one per applicable Pursuit, plus one per
  applicable Background, per Devil's Bargain accepted, and per ally Helping.
  DARK rolls the GM-assigned dark dice and counts each die *under* Risk. The
  widget opens on **DARK** and remembers your last pick for the session.
- **The widget cannot roll light and dark together.** "Highest dark beats highest
  light" is not expressible in a single stance-modal-pool roll. Roll the two
  pools separately and let the GM adjudicate the comparison — the GM agent's
  prompt carries the dark-overrule guidance for exactly this.
- **A die that lands exactly on Risk is EDGE OF RUIN.** It does **not** count as a
  success; it's a hair's-breadth beat the GM narrates ("the wood almost takes
  you"). The tag reports it as `exactMatches=N`.
- **Outcome tiers** are named in the tag: 0 successes `devils-bargain`, 1
  `at-a-cost`, 2 or more `clean`.
- **Ruin (0-6) and Burdens** live on the sheet. The widget does not tick them —
  the State Mutator does that from the narration. At **Ruin 6** the character is
  consumed by the wood.

## Sanity check

In a fresh Game Mode chat:

1. Click the dice widget icon. The widget renders the stance form: DARK / LIGHT,
   Pool, Risk, **Roll Nxd6**.
2. Click **LIGHT**. Set Pool = 3, Risk = 3. Click the roll button.
3. You should see a tag like `[mrr-roll: ruleset=trophy-dark, stance=light, stat=Risk, statValue=3, pool=3, dice=[2,4,5], successes=2, exactMatches=0, tier=clean]`
   (your dice will vary).
