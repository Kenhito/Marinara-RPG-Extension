...
### 2. Install the GURPS Lite bundle

Click the **Ruleset** button. The dialog has two ways to load a bundle:

- **Option A — Choose file:** click **Choose file…** and pick
  `rulesets/gurps-lite/bundle.json` from disk. Click **Save and reload**.
- **Option B — Fetch URL:**
  `https://raw.githubusercontent.com/Kenhito/Marinara-RPG-Extension/main/rulesets/gurps-lite/bundle.json`

This one import installs the ruleset (sheet + dice widget), the lorebook
("GURPS Lite Rules Reference", 43 entries), the main GM agent, three sub-agents,
the `gurps_lite_reference` custom tool, and two regex scripts that surface
skill-check tags and difficulty mentions in chat. Bundles are data — no extension
re-approval is triggered. The page reloads with the ruleset active.
...
2. **Enable the MRR agents for the game** — after it launches, not
   ...
   `MRR: GURPS Lite Ruleset Helper` and `MRR: GURPS Lite — <Role>`
...
grant to make. Without it the narrating model *invents* every skill roll, defense
roll, and damage total by picking a plausible number. GURPS is a bell curve —
3d6 is not uniform, and a model guessing "a plausible number" does not reproduce
that curve, so the difficulty of everything quietly drifts. The same toggle gates
this bundle's `gurps_lite_reference` custom tool.
...
The widget is a roll-under form: **Target**, **Bonus**, and a **Roll 3d6**
button. Enter the effective skill or attribute level as Target; success is a
total at or under it, and the tag reports the margin of success or failure.

- **Modifiers go in the Bonus field**, not into Target — that keeps the tag
  honest about what the base number was.
- **Critical success** is a total of 4 or less; **critical failure** is a failed
  roll 10 or more over the target. Both are labelled in the tag.
- **Skill defaults are not computed.** The difficulty ladder (Easy through Very
  Hard, attribute minus 4 through minus 9) lives in the lorebook — look the
  default up and enter the resulting number.
- **Hit Points and Fatigue Points** live on the sheet as tracked resources; the
  widget does not spend or restore them. The State Mutator does that from the
  narration.
...
1. Click the dice widget icon. The widget renders the roll-under form: Target,
   Bonus, **Roll 3d6**.
2. Set Target = 14, Bonus = 0. Click **Roll 3d6**.
3. You should see a tag like `[dice: 3d6 vs 14 -> 9 (3+2+4) SUCCESS, margin 5]`
   (your dice will vary).
