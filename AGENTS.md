...
1. **GM-mode agents bundled and auto-installed.** `tools/build-bundle.mjs` produces `bundle.json` containing the ruleset, lorebook, `gmAgent` (the main Ruleset Helper), AND `additionalAgents[]` (the role agents). All sub-agents ship `enabled:true` because GM-mode has no per-agent toggle UI — bundles must arrive installed-and-live. `tools/build-agents.mjs` also produces a standalone `agents.json` for parity with the RP-mode toolchain, but in GM-mode the bundle is the install path. Users migrating from v0.4.x legacy installs uninstall the ruleset and reinstall the new bundle; the extension's `mrrAgentRole` idempotency keying matches agents across the re-install without accumulating duplicates.
