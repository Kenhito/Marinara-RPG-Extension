...
| Sync | `scheduleAutoSync` → `syncSheetToAgents` (agent promptTemplate sheet block — the load-bearing read path) + `syncFieldReferenceToLorebook` (managed lorebook field-reference entry). The former third leg, `syncSheetToChat`, was removed in round 27: the engine has Zod-stripped `customTrackerFields` from `PATCH /chats/:id` since 2.0.9, so it wrote nothing. |
