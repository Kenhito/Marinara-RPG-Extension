...
- **The Coupling Law:** any commit that adds/removes/changes an engine touchpoint (REST route, DOM selector, storage key, API shape) MUST update `COUPLINGS.md` (repo root) in the same commit. Internal (Class-C) couplings require a feature probe, a graceful fallback, and a visible degradation warning in the MRR panel. See `COUPLINGS.md` for the full ledger and `Plans/2026-08-22_engine-2.4.3-upgrade-and-rolemaster-plan.md` §1.3/§3.
