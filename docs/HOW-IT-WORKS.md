...
- **Roleplay mode only:** if your preset has no slot reserved for a given agent's output, that agent's work is silently discarded — it still runs, still costs tokens, and the narrator never sees a word of it. Use **Manage MRR Agents → Add agent sections to active preset** once per preset, and run it again from inside every **new chat** before the first turn — a new chat can come up without that preset bound to it, and until it is, the narrator reads nothing from the agents even though the sheet-writer still works. The dialog is safe to re-run: it tells you "nothing to do" when the chat is already wired. Later bundle updates repair the sections themselves automatically after that first pass.
...
| An enabled agent seems to be doing nothing (roleplay mode) | Your preset has no matching section reserved for that agent's output | **Manage MRR Agents → Add agent sections to active preset** — once per preset, and again from inside each new chat before its first turn (a new chat may not have the preset bound yet); safe to re-run, and the tell-tale is a narrator that writes to the sheet fine but never quotes it |
