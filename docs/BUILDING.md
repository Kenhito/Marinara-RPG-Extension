# BUILDING — the generic bundle-build contract
...
> - `releases/1.1.0/docs-for-ai/` — release-pinned build-pipeline reference (this doc supersedes for newer schema features)
>
...
| `resolution.direction` | string | — | dice widget | `d100-percentile` only. `"under"` (default, back-compatible) = success when the roll is at or under the target. `"high"` = success when roll + bonus meets or beats the target; the widget reports a total rather than a pass/fail against a percentile cap. Rolemaster and other roll-high d100 systems use `"high"`. |
| `resolution.bonusFormula` | string | — | (informational) | `d100-percentile` only. Plain-language description of what the player adds to a `"high"` roll (e.g. `"skill bonus + stat bonus + item bonus"`). **Descriptive only — the widget takes the bonus as a numeric input; it does not parse this string.** |
| `resolution.openEnded` | object | — | dice widget | `d100-percentile` only. Open-ended (exploding, and optionally imploding) d100; absent = feature off. `high.threshold` (default 96) re-rolls and ADDS at or above that face; `low.threshold` (default 5) triggers the low chain, `low.subtract` (default true) subtracts it, `low.continueOn` (default `"high"`) says which follow-up faces extend it (RMSS is asymmetric — the low chain continues while each new roll is at or **above** `openEnded.high.threshold`). Both chains take a `cascadeCap` (default `0` = uncapped). `unusualFaces` is an optional integer array of UNMODIFIED-first-roll faces that flag `um=<face>` on the tag (RMSS: `[66, 100]`) — no default, so absent or empty means no `um=` is ever emitted. **The widget only ever surfaces the flag — the narrator decides what an UM means; the widget never picks a table row and never suppresses the roll.** Full shape + worked example in [`docs/AUTHORING-PHASE-6.md`](AUTHORING-PHASE-6.md) §1. |
...
**Idempotency contract:** at install time, the extension prefixes each script's name with `MRR: <rulesetId>: ` (GM-mode) / `MRRP: <rulesetId>: ` (RP-mode). Re-install does GET → filter by prefix → DELETE matches → POST fresh. Hand-edited managed scripts get overwritten on re-install (matches the lorebook contract).
...
- Tool name must match `^[a-z][a-z0-9_]*$` (lowercase snake_case, ≤100 chars). The generator emits compliant names; the installer prefixes with `mrr_<rulesetIdSnake>_` (GM-mode) / `mrrp_<rulesetIdSnake>_` (RP-mode) preserving the regex.
...
**Idempotency contract:** at install time, the extension prefixes each tool's name with `mrr_<rulesetIdSnake>_`. Re-install does GET → filter by prefix → DELETE matches → POST fresh. Same DELETE-then-POST trade-off as the regex installer (non-atomic; retry recovers).
...
- `mrrRenderIdentitySubField` detects `key === "class"` + `ruleset.classOptions[]` and renders a `<select>` instead of `<input>`. Empty placeholder option appears when no class is selected; selection persists in `state.sheet.identity.class` (same storage as the legacy free-text path, so save / load / character-card migration paths are unchanged).
...
- `mrrResolveResourceDie(resource)` is the single source of truth for die-substitution: returns `resource.die` (fallback) unless `dieFromClass: true` AND `classOptions[]` is declared AND the selected class is present in `classOptions[]` with a `hitDie` string. RP-mode has the same helper as `mrrpResolveResourceDie`.
...
| Agent | Phase | Source | Responsibility |
|-------|-------|--------|----------------|
...
| `state-mutator` | `post_processing` | `agents/state-mutator.md` | The sole sheet-writer. Runs AFTER the narration, reads the completed turn (`context.mainResponse`), and emits `[mrr-state: ...]` tags copying the numbers the GM wrote; the extension applies them from the persisted custom-agent runs the poller reads. **A per-ruleset override must declare `**Phase:** post_processing` itself** — `extractPhase()` reads the override file when one exists, and an override with no `**Phase:**` line falls back to `pre_generation`. |
...
Some rulesets ship additional **`parallel`-phase** agents that track system-specific resources alongside the main narrator without blocking it. They live ONLY at `rulesets/<id>/agents/<role>.md` — no shared baseline — because the resources they track exist only in that system.
...
A standard ruleset (no parallel overlays) fires **3 sub-agent calls** per turn — combat-overseer and context-fuser as `pre_generation` context injectors, plus state-mutator as a `post_processing` call after the narration completes (its *effects* land when the extension's runs-endpoint poller reads its emitted `[mrr-state:]` tags). Total call count is unchanged by the round-25 phase move; only when the third one fires changed. Rulesets with parallel overlays add those calls outside the critical path (parallel phase doesn't block the next narration on providers that allow concurrent calls — on a 1-call-at-a-time provider every agent serializes, so count matters; see the plan's agent-budget item). The `pre-input-transformer` when present adds one more pre-gen call. The main Ruleset Helper (`gmAgent`) is a further pre-gen call on top of the sub-agents.
...
A ruleset can override `combat-overseer`, `context-fuser`, `state-mutator`, or the `main` prompt by dropping `rulesets/<id>/agents/<role>.md` or `rulesets/<id>/gm-agent.md` into the ruleset directory. `build-agents.mjs` and `build-bundle.mjs` both prefer the override path when present; the shared baseline applies otherwise.
